...
public class Events implements EventListener {
    ...
    public void Tick(EnumSet<TickEvent.Type> type, Object... tickData) {
        if (type.contains(TickEvent.Type.PLAYER)) {
            ...
            ItemStack boots = player.inventory.armorItemInSlot(3);
                if (XEnchants.containsEnchant(boots, Config.enchFlyID))
                {
                    Fly = true;
    ...
    public void onItemTooltip(ItemTooltipEvent event) {
    	if (Config.Tooltip){
       if (event.itemStack.getItem() instanceof ItemArmor && XEnchants.containsEnchant(event.itemStack, Config.enchFlyID)) {
           event.toolTip.add(COLOR + "9Double jump to Fly");
           event.toolTip.add(COLOR + "3Boots absorb all fall damage");
       }
       if (event.itemStack.getItem() instanceof ItemArmor && XEnchants.containsEnchant(event.itemStack, Config.enchSwiftnessID)) {
       ...
       if (event.itemStack.getItem() instanceof ItemArmor && XEnchants.containsEnchant(event.itemStack, Config.enchVitalityID)) {
       ...
       if (event.itemStack.getItem() instanceof ItemArmor && XEnchants.containsEnchant(event.itemStack, Config.enchBattleHealingID)) {
       ...
       if (event.itemStack.getItem() instanceof ItemArmor && XEnchants.containsEnchant(event.itemStack, Config.enchWitherProtectionID)) {
       ...
       if (event.itemStack.getItem() instanceof ItemArmor && XEnchants.containsEnchant(event.itemStack, Config.enchAdrenalineBoostID)) {
